...
import { FaArrowLeft, FaDownload, FaSearch, FaUser } from 'react-icons/fa';
import { getMaterialesPorUsuario } from '../../../api/informes.api';
...
const MaterialesPorUsuario: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await getMaterialesPorUsuario();
        setMateriales(data);
        setError(null);
      } catch (err) {
  ...
  const filteredMateriales = materiales.filter((material) => {
    ...
    return (
      ...
      material.nombre.toLowerCase().includes(searchString) ||
      material.apellido.toLowerCase().includes(searchString) ||
      material.email.toLowerCase().includes(searchString) ||
      material.tipo_movimiento.toLowerCase().includes(searchString)
    );
  });
  ...
  const csvData = [
    ['Código SENA', 'Nombre Material', 'Descripción', 'Usuario Responsable', 'Email', 'Tipo Movimiento', 'Fecha'],
    ...filteredMateriales.map((material) => [
      material.codigo_sena,
      material.nombre_material,
      material.descripcion_material,
      `${material.nombre} ${material.apellido}`,
      material.email,
      material.tipo_movimiento,
      new Date(material.fecha_creacion).toLocaleString()
    ])
  ];
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center">
          ...
          <h1 className="text-2xl font-bold text-gray-800">Materiales Asignados por Usuario</h1>
        </div>
        <div className="flex items-center">
          <CSVLink
            data={csvData}
            filename="materiales_por_usuario.csv"
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <div className="flex items-center mb-4">
          <div className="relative flex-grow">
            <input
              type="text"
              placeholder="Buscar por nombre, código, usuario..."
                ...
                <tbody className="text-gray-600 text-sm">
                  {currentItems.length > 0 ? (
                    currentItems.map((material) => (
                      <tr key={`${material.id_movimiento}-${material.material_id}`} className="border-b border-gray-200 hover:bg-gray-50">
                        ...
                        <td className="py-3 px-6 text-left flex items-center">
                          <div className="mr-2 bg-blue-100 p-2 rounded-full">
                            <Icon icon={FaUser} size={16} className="text-blue-500" />
                          </div>
                          {material.nombre} {material.apellido}
                        </td>
                        <td className="py-3 px-6 text-left">{material.email}</td>
                        <td className="py-3 px-6 text-left">
                          <span className={`px-2 py-1 rounded-full text-xs ${
                            material.tipo_movimiento.includes('Entrada') ? 'bg-green-100 text-green-800' :
                            material.tipo_movimiento.includes('Salida') ? 'bg-red-100 text-red-800' :
                            'bg-blue-100 text-blue-800'
                          }`}>
                            {material.tipo_movimiento}
                          </span>
                        </td>
                        <td className="py-3 px-6 text-left">{new Date(material.fecha_creacion).toLocaleString()}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
...
export default React.memo(MaterialesPorUsuario);
