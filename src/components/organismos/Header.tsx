...
const Header = () => {
  ...
  // Obtener la imagen del usuario sin perder la que ya estaba si los datos detallados aún no traen imagen
  // Priorizamos cualquier imagen válida de usuarioDetalles y, si no existe, usamos la del authState.user
  // @ts-ignore - Ignorar error de TypeScript por posibles claves distintas
  const detailedImg = usuarioDetalles?.imagen ?? usuarioDetalles?.avatar ?? usuarioDetalles?.foto;
  // @ts-ignore
  const baseImg = authState.user?.imagen ?? authState.user?.avatar ?? authState.user?.foto;
  const userImage = (detailedImg ?? baseImg) ?? "/assets/default.jpg";

  // Normalizar la URL de la imagen de forma segura
  // Mantener si ya es absoluta (http/https), data URL o ruta absoluta (/...)
  const finalUserImage =
    typeof userImage === 'string'
      ? (userImage.startsWith('http') || userImage.startsWith('data:') || userImage.startsWith('/')
          ? userImage
          : `https://${userImage}`)
      : "/assets/default.jpg";
