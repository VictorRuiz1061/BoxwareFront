...
import { setJsonCookie } from '@/api/axiosConfig';
...
export const login = async (credentials: LoginCredentials): Promise<AuthResponse> => {
  try {
    ...
    setJsonCookie('user', processedUser);
...
export const hasValidToken = (): boolean => {
  ...
  try {
    ...
    if (parts.length !== 3) {
      console.log('El token no tiene formato JWT válido (debe tener 3 partes)');
      return false;
    }
    
    const payloadBase64 = parts[1];
    // Asegurarse de que el base64 esté correctamente formateado para atob
    const base64 = payloadBase64.replace(/-/g, '+').replace(/_/g, '/');
    const payload = JSON.parse(atob(base64));
    
    console.log('Payload del token:', payload);
    console.log('Tiempo actual (segundos):', Math.floor(Date.now() / 1000));
    console.log('Tiempo de expiración del token:', payload.exp);
    
    // Verificar que exp exista y sea mayor que el tiempo actual
